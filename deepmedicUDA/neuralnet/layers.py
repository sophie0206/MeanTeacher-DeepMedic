...
from deepmedicUDA.neuralnet.ops import applyDropout, makeBiasParamsAndApplyToFms, makeDDBiasParamsAndApplyToFms, applyRelu, applyPrelu, applyElu, applySelu, pool3dMirrorPad
from deepmedicUDA.neuralnet.ops import applyBn, createAndInitializeWeightsTensor, convolveWithGivenWeightMatrix, applySoftmaxToFmAndReturnProbYandPredY
...
class SoftmaxLayer(TargetLayer):
    ...
    def makeLayer(  self,
        ...
        ##=============================Only Bseg batchsize can do classification=============================================##
        outputFcTrain, outputFcVal, outputFcTest = layerConnected.output["train"], layerConnected.output["val"], layerConnected.output["test"]
        outputBsegTrain = outputFcTrain[:batchSize_seg["train"],:,:,:,:]
        outputBsegVal = outputFcVal[:batchSize_seg["train"],:,:,:,:]
        outputBsegTest = outputFcTest

        shapeTr, shapeVal, shapeTe = layerConnected.outputShape["train"], layerConnected.outputShape["val"], layerConnected.outputShape["test"]
        outputBsegTrainShape = [batchSize_seg["train"]] + shapeTr[1:]
        outputBsegValShape = [batchSize_seg["val"]] + shapeVal[1:]
        outputBsegTestShape = [batchSize_seg["test"]*2] + shapeTe[1:]

        
        self._setBlocksInputAttributes(outputBsegTrain, outputBsegVal, outputBsegTest, 
