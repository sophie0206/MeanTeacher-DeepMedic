...
from deepmedicUDA.neuralnet.pathwayTypes import PathwayTypes
from deepmedicUDA.neuralnet.utils import calcRecFieldFromKernDimListPerLayerWhenStrides1
from deepmedicUDA.neuralnet.layers import ConvLayer, LowRankConvLayer, FcLayer
...
def makeResidualConnectionBetweenLayersAndReturnOutput( log,
    ...
    log.print3("###########################=======############### \n In make residual connections function shape of partofEarlier" + str(earlierLayerOutputImageTrain.shape[0]) )
...
class Pathway(object):
    ...
    def __init__(self, pName=None) :
        ...
        #==============Output to Domain Discriminator================##
        self._outputDD = {"train": None, "val": None, "test": None}
        self._outputDDShape = {"train": None, "val": None, "test": None}
    
    ##===================================ADDED==================================================##
    def makeConcatedActivations(self,
                                log,
                                deeperLayerOutputImagesTrValTest,
                                deeperLayerOutputImageShapesTrValTest,
                                earlierLayerOutputImagesTrValTest,
        ...
        ##======================================================================================================##
        outputOfDDTrain = tf.concat([deeperLayerOutputImageTrain, partOfEarlierFmsToAddTrain], axis=1)
        outputOfDDVal = tf.concat([deeperLayerOutputImageVal, partOfEarlierFmsToAddVal], axis=1)
        outputOfDDTest = tf.concat([deeperLayerOutputImageTest, partOfEarlierFmsToAddTest], axis=1)

        numFmstotalTrain = deeperLayerOutputImageShapeTrain[1] + earlierLayerOutputImageShapeTrain[1]
        numFmstotalVal = deeperLayerOutputImageShapeVal[1] + earlierLayerOutputImageShapeVal[1]
        numFmstotalTest = deeperLayerOutputImageShapeTest[1] + earlierLayerOutputImageShapeTest[1]

        outputOfDDShapeTrain = [deeperLayerOutputImageShapeTrain[0]] + [numFmstotalTrain] + deeperLayerOutputImageShapeTrain[2:]
        outputOfDDShapeVal = [deeperLayerOutputImageShapeVal[0]] + [numFmstotalVal] + deeperLayerOutputImageShapeVal[2:]
        outputOfDDShapeTest = [deeperLayerOutputImageShapeTest[0]] + [numFmstotalTest] + deeperLayerOutputImageShapeTest[2:]

        ##=======================================================================================================## 
        self._setOutputAttributesForDD(outputOfDDTrain, outputOfDDVal, outputOfDDTest, 
                                       outputOfDDShapeTrain, outputOfDDShapeVal, outputOfDDShapeTest)
    ...
    def makeLayersOfThisPathwayAndReturnDimensionsOfOutputFM(self,
        ...
        for layer_i in range(0, numOfLayers) :
            ...
            if layer_i not in indicesOfLayersToConnectResidualsInOutputForPathway : #not a residual connecting here
                ...
                ##=======================================================================================##
                # The layers they are going to make residual connections are the exact same layers we are going to
                # Process their activations. To grab training batches belong to B_adv.

                outTrain = layer.output["train"]
                outVal = layer.output["val"]
                outTest = layer.output["test"]

                outTrainShape = [(batchSize_adv["train"]*2)] + layer.outputShape["train"][1:]
                outValShape = [(batchSize_adv["val"]*2)] + layer.outputShape["val"][1:]
                outTestShape = [(batchSize_adv["test"]*2)] + layer.outputShape["test"][1:]

                DDdeeperLayerOutputImagesTrValTest = (outTrain, outVal, outTest)
                DDdeeperLayerOutputImageShapesTrValTest = (outTrainShape, outValShape, outTestShape)

                activationsForDD.append(DDdeeperLayerOutputImagesTrValTest)
                activationShapesForDD.append(DDdeeperLayerOutputImageShapesTrValTest)
                ##=======================================================================================##
        ...
        inputShapeDDTrain = layer.outputShape["train"] # from attribute in the layers.py
        inputShapeDDVal = layer.outputShape["val"]
        inputShapeDDTest = layer.outputShape["test"]       
        ...
        ##============================Concate and Crop the Activations of 4,6,8 Layers========================================## 
        # Here We set the final concated output in the makeConcatedActivations function using the built-in func.
        # The normal pathway and the subsampled pathway are the same when simply concating activations.
        # When it comes to Fc layers, there is only one Fc activation coming from layer 2 in Fc pathway.
        # And it will be set as the outputForDD of this pathway.

        earlierOutputAll = activationsForDD[0]
        earlierOutputShapesAll = activationShapesForDD[0]
        log.print3("In concatenate process, the first !!"+str(earlierOutputShapesAll[0]))
        self._setOutputAttributesForDD(earlierOutputAll[0], earlierOutputAll[1], earlierOutputAll[2],
                                    earlierOutputShapesAll[0], earlierOutputShapesAll[1], earlierOutputShapesAll[2])

        for i in range(len(activationsForDD)):
            if i > 0:
                thisLayerOutputAll = activationsForDD[i]
                thisLayerOutputShapesAll = activationShapesForDD[i]
                self.makeConcatedActivations(log,
                            thisLayerOutputAll,
                            thisLayerOutputShapesAll,
                            earlierOutputAll,
                            earlierOutputShapesAll)

                earlierOutputAll = self.getOutputDD() # return a list while in the function, they treat this param as a tuple.
                earlierOutputShapesAll = self.getShapeOfOutputDD() # and it turns out to be okay.
                log.print3("In concatenate process!!"+str(thisLayerOutputShapesAll[0]))
        ##=======================================================================================##
        ...
        log.print3("\t[Pathway_"+"DD"+"]: Output's Shape: (Train) " + str(self._outputDDShape["train"]) + \
                        ", (Val) " + str(self._outputDDShape["val"]) + ", (Test) " + str(self._outputDDShape["test"]))
    ...
    ##=====================================ADDED=====================================##
    def _setOutputAttributesForDD(self, outputTrain, outputVal, outputTest, outputShapeTrain, outputShapeVal, outputShapeTest):
        self._outputDD["train"] = outputTrain; self._outputDD["val"] = outputVal; self._outputDD["test"] = outputTest
        self._outputDDShape["train"] = outputShapeTrain; self._outputDDShape["val"] = outputShapeVal; self._outputDDShape["test"] = outputShapeTest
    ##=============================================================================##  
    # Getters
    def pName(self):
        return self._pName
    ...
    ##==================ADDED=================================
    def getOutputDD(self):
        return[self._outputDD["train"],self._outputDD["val"],
               self._outputDD["test"]]
    
    def getShapeOfOutputDD(self):
        return [ self._outputDDShape["train"], self._outputDDShape["val"], self._outputDDShape["test"] ]
    ##========================================================
...
class DDPathway(Pathway):
    def __init__(self, pName=None) :
        Pathway.__init__(self, pName)
        self._pType = PathwayTypes.DD

    def getStringType(self) :
        return "DD"


    #=================================== ADDED Part: makelayers for DD Pathway===================#
    def makeLayersOfDDPathwayAndReturnDimensionsOfOutputFM(self,
                                                    log,
                                                    ...
                                                    DDnumKernsPerLayer, # A list contains number of kernels of every layer
                                                    DDkernelDimsPerLayer,
                                                    
                                                    DDconvWInitMethod,
                                                    DDuseBnPerLayer, # As a flag for case that I want to apply BN on input image. I want to apply to input of FC.
                                                    DDmovingAvForBnOverXBatches,
                                                    DDactivFuncPerLayer,
                                                    DDdropoutRatesPerLayer=[],
                                                    
                                                    DDpoolingParamsStructureForThisPathwayType = [],
                                                    ) :
        rng = np.random.RandomState(55789)
        log.print3("[Pathway_" + str(self.getStringType()) + "] is being built...")
        
        self._recField = self.calcRecFieldOfPathway(DDkernelDimsPerLayer)
        
        self._setInputAttributes(DDinputTrain, DDinputVal, DDinputTest, DDinputDimsTrain, DDinputDimsVal, DDinputDimsTest)                
        log.print3("\t[Pathway_"+str(self.getStringType())+"]: Input's Shape: (Train) " + str(self._inputShape["train"]) + \
                ", (Val) " + str(self._inputShape["val"]) + ", (Test) " + str(self._inputShape["test"]))
        
        inputToNextLayerTrain = self._input["train"]; inputToNextLayerVal = self._input["val"]; inputToNextLayerTest = self._input["test"]
        inputToNextLayerShapeTrain = self._inputShape["train"]; inputToNextLayerShapeVal = self._inputShape["val"]; inputToNextLayerShapeTest = self._inputShape["test"]
        numOfLayers = len(DDnumKernsPerLayer)
        
        for layer_i in range(0, numOfLayers) :
            thisLayerFilterShape = [DDnumKernsPerLayer[layer_i],inputToNextLayerShapeTrain[1]] + DDkernelDimsPerLayer[layer_i]
            
            thisLayerUseBn = DDuseBnPerLayer[layer_i]
            thisLayerActivFunc = DDactivFuncPerLayer[layer_i]
            thisLayerDropoutRate = DDdropoutRatesPerLayer[layer_i] if DDdropoutRatesPerLayer else 0
            
            thisLayerPoolingParameters = DDpoolingParamsStructureForThisPathwayType[layer_i]
            ...
            layer = ConvLayer()
            layer.makeDDLayer(rng,
                            inputToNextLayerTrain,
                            inputToNextLayerVal,
                            inputToNextLayerTest,
                            inputToNextLayerShapeTrain,
                            inputToNextLayerShapeVal,
                            inputToNextLayerShapeTest,
                            
                            thisLayerFilterShape,
                            thisLayerPoolingParameters,
                            DDconvWInitMethod,
                            thisLayerUseBn,
                            DDmovingAvForBnOverXBatches,
                            thisLayerActivFunc,
                            thisLayerDropoutRate
                            ) 
        ...
        layerfc.makeFcLayer(inputToNextLayerTrain,inputToNextLayerVal, inputToNextLayerTest,inputToNextLayerShapeTrain,inputToNextLayerShapeVal,inputToNextLayerShapeTest,rng)

        self._layersInPathway.append(layerfc)

        inputToNextLayerTrain = layerfc.output["train"]
        inputToNextLayerVal = layerfc.output["val"]
        inputToNextLayerTest = layerfc.output["test"]
            
        inputToNextLayerShapeTrain = layerfc.outputShape["train"]
        inputToNextLayerShapeVal = layerfc.outputShape["val"]
        inputToNextLayerShapeTest = layerfc.outputShape["test"]
...
class SubsampledPathway(Pathway):
    ...
    def upsampleOutputToNormalRes(self, upsamplingScheme="repeat",
                            ...
                            shapeToMatchForDDTrain=None, shapeToMatchForDDVal=None, shapeToMatchForDDTest=None):
        ...
        ##=================================ADDED to upsample For DD=============================##
        [outputDDTrain, outputDDVal, outputDDTest] = self.getOutputDD()
        [outputDDShapeTrain, outputDDShapeVal, outputDDShapeTest] = self.getShapeOfOutputDD()

        outputOfUpsampledTrain = upsampleRcz5DimArrayAndOptionalCrop(outputDDTrain,
                                                                     outputDDShapeTrain,
                                                                     self.subsFactor(),
                                                                     upsamplingScheme,
                                                                     shapeToMatchForDDTrain)
        outputOfUpsampledVal = upsampleRcz5DimArrayAndOptionalCrop(outputDDVal,
                                                                   outputDDShapeVal,
                                                                   self.subsFactor(),
                                                                   upsamplingScheme,
                                                                   shapeToMatchForDDVal)

        outputOfUpsampledTest = upsampleRcz5DimArrayAndOptionalCrop(outputDDTest,
                                                                   outputDDShapeTest,
                                                                   self.subsFactor(),
                                                                   upsamplingScheme,
                                                                   shapeToMatchForDDTest)

        DDUpsampledShapeTrain = outputDDShapeTrain[:2] + shapeToMatchForDDTrain[2:]
        DDUpsampledShapeVal = outputDDShapeVal[:2] + shapeToMatchForDDVal[2:]
        DDUpsampledShapeTest = outputDDShapeTest[:2] + shapeToMatchForDDTest[2:]

        self._setOutputAttributesForDD(outputOfUpsampledTrain, outputOfUpsampledVal,outputOfUpsampledTest,
                                       DDUpsampledShapeTrain, DDUpsampledShapeVal, DDUpsampledShapeTest)
        ##================================================================================##
...
class FcPathway(Pathway):
    ...
    def getStringType(self) :
        return "FC"
##============================Useless now==============================##
    def grabBsegOutputForClassification(self, batchSize_seg):
        outputFcTrain, outputFcVal, outputFcTest = self.getOutput()
        outputBsegTrain = outputFc[:,:batchSize_seg["train"],:,:,:]
        outputBsegVal = outputFc[:,:batchSize_seg["val"],:,:,:]
        outputBsegTest = outputFc[:,:batchSize_seg["test"],:,:,:]

        shapeTr, shapeVal, shapeTe = self.getShapeOfOutput()
        outputBsegTrainShape = [shapeTr[0], batchSize_seg["train"]] + shapeTr[2:]
        outputBsegValShape = [shapeVal[0], batchSize_seg["val"]] + shapeVal[2:]
        outputBsegTestShape = [shapeTe[0], batchSize_seg["test"]] + shapeTe[2:]

        self._setOutputAttributes(outputBsegTrain, outputBsegVal, outputBsegTest, 
                                outputBsegTrainShape, outputBsegValShape, outputBsegTestShape)
