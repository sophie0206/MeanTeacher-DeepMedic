...
def DDx_entr( p_y_given_x_train, y_gt, weightPerClass ):
    # p_y_given_x_train : tensor2 [batchSize, classes]
    # y: T.itensor1('y'). Dimensions [batchSize,]
    # weightPerClass is a vector with 1 element per class.
    ...
    weightPerClass2D = tf.reshape(weightPerClass, shape=[1, tf.shape(p_y_given_x_train)[1]])
    weighted_log_p_y_given_x_train = log_p_y_given_x_train * weightPerClass2D
...
def getAlphaOfThisEpoch(epoch):
    alpha_max = tf.constant(0.05, dtype='float64')
    e2 = tf.constant(35, dtype='int64')
    e1 = tf.constant(10, dtype='int64')
    
    alpha = tf.cond(epoch < e1, lambda: tf.constant(0.0, dtype='float64'), lambda: alpha_max * ((epoch - e1) / (e2 - e1)))

    return tf.to_float(alpha)
##==========================================================================================##
