...
from deepmedicUDA.logging.accuracyMonitor import AccuracyOfEpochMonitorSegmentation
from deepmedicUDA.neuralnet.wrappers import CnnWrapperForSampling
from deepmedicUDA.dataManagement.sampling import getSampledDataAndLabelsForSubepoch, getTDSampledDataAndLabelsForSubepoch
from deepmedicUDA.routines.testing import performInferenceOnWholeVolumes

from deepmedicUDA.logging.utils import datetimeNowAsStr
...
def doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(log,
    ...
    ##====================================================================##
    DDarrayWithNumbersOfPerClassRpRnTpTnInSubepoch = np.zeros([ 2, 4 ], dtype="int32")
    ##====================================================================##
    ...
    for batch_i in range(number_of_batches):
        ...
        if train_or_val=="train" :
            ...
            list_of_ops = [ ops_to_fetch['cost'] ] + ops_to_fetch['list_rp_rn_tp_tn'] + ops_to_fetch['DDlist_rp_rn_tp_tn'] + [ ops_to_fetch['updates_grouped_op'] ]
            ##==================================================================##
            ...
            index_to_data_for_batch_min_adv = batch_i * cnn3d.batchSize_adv["train"]
            index_to_data_for_batch_max_adv = (batch_i+1) * cnn3d.batchSize_adv["train"]
            
            log.print3(str(index_to_data_for_batch_max_seg) + " " + str(index_to_data_for_batch_max_adv))
            ...
            #print("In training"+str(thisBatchAlldata.shape))
            ...
            LabelsForBadv = [1] * cnn3d.batchSize_adv["train"] + [0] * cnn3d.batchSize_adv["train"]
            feeds_dict.update( { feeds['yDD_gt']: LabelsForBadv})
            ...
            #log.print3(str(listWithCostMeanErrorAndRpRnTpTnForEachClassFromTraining))
            
            listWithNumberOfRpRnPpPnForEachClass = listWithCostMeanErrorAndRpRnTpTnForEachClassFromTraining[1:9] #21
            
            DDlistWithNumberOfRpRnPpPnForEachClass = listWithCostMeanErrorAndRpRnTpTnForEachClassFromTraining[9:]
        ...
        DDarrayWithNumberOfRpRnPpPnForEachClassForBatch = np.asarray(DDlistWithNumberOfRpRnPpPnForEachClass, dtype="int32").reshape(DDarrayWithNumbersOfPerClassRpRnTpTnInSubepoch.shape, order='C')
        ...
        DDarrayWithNumbersOfPerClassRpRnTpTnInSubepoch += DDarrayWithNumberOfRpRnPpPnForEachClassForBatch
    ...
    accuracyMonitorForEpoch.DDupdateMonitorAccuraciesWithNewSubepochEntries(meanCostOfSubepoch, DDarrayWithNumbersOfPerClassRpRnTpTnInSubepoch)
    
    accuracyMonitorForEpoch.DDreportAccuracyForLastSubepoch()
    ##========================================================================##
    #Done
...
def do_training(sessionTf,
        ...
        DDlistOfFilepathsToGtLabelsOfEachPatientValidationOnSamplesAndDsc,
                ...
                #-------Sampling Type---------
                samplingTypeInstanceTraining, # Instance of the deepmedicUDAEN/samplingType.SamplingType class for training and validation
...
##===============================================================================================================##
    ...
    tupleWithModulesToImportWhichAreUsedByTheJobFunctions = ( "from __future__ import absolute_import, print_function, division",
                "time", "numpy as np", "from deepmedicUDAEN.dataManagement.sampling import *" )
        ...
        for subepoch in range(number_of_subepochs): #per subepoch I randomly load some images in the gpu. Random order.
            ...
            if performValidationOnSamplesDuringTrainingProcessBool :
                if boolItIsTheVeryFirstSubepochOfThisProcess :
                    ...
                    TDlabelsForCentralOfSegmentsForSubepVal] = getTDSampledDataAndLabelsForSubepoch(log,
                                                                        "val",
                                                                        run_input_checks,
                                                                        cnn3dWrapper,
                                                                        maxNumSubjectsLoadedPerSubepoch,
                ...
                ##=====================================================================================##
                    [TDchannsOfSegmentsForSubepPerPathwayVal,
                    TDlabelsForCentralOfSegmentsForSubepVal] = TDparallelJobToGetDataForNextValidation() #fromParallelProcessing that had started from last loop when it was submitted.
                ...
                doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(log,
                                                                            ...
                                                                            TDlabelsForCentralOfSegmentsForSubepVal)
...
#SDlabelsForCentralOfSegmentsForSubepTrain,
