...
from deepmedicUDA.logging.utils import strFl4fNA, strFl5fNA, strListFl4fNA, strListFl5fNA, getMeanOfListExclNA
...
class AccuracyOfEpochMonitorSegmentation(object) :
    ...
    def __init__(self,
        ...
        self.DDnumberOfClasses = 2
        self.DDnumberOfSubepochsPerEpoch = numberOfSubepochsPerEpoch
        
        self.DDnumberOfSubepochsForWhichUpdated = 0
        
        # === Collecting accuracy metrics over the whole epoch: ===
        # --- mean Empirical Accuracy: number of Corr Classified voxels and all voxels, wrt to all the classes, in multiclass problems.---

        self.DDcorrectlyPredVoxelsInEachSubep = []
        self.DDnumberOfAllSamplesOfEachSubep = []
        self.DDmeanEmpiricalAccuracyOfEachSubep = [] # mean (multiclass) accuracy. correctlyPredVoxelsInEachSubep / numberOfAllSamplesOfEachSubep # NOTE: Cannot contain N0T-APPLICABLE
        
        # --- Per Class Accuracies and Real/True Pos/Neg (in a One-Vs-All fashion)
        # These do not have the class-0 background flipped to foreground!
        
        self.DDlistPerSubepPerClassRpRnTpTn = [] # subepochs X Classes X 4. 4 = RP, RN, TP, TN
        self.DDlistPerSubepPerClassMeanAccSensSpecDsc = [] # NOTE: May contain N0T-APPLICABLE=self.NA_PATTERN elements, eg when class not present!
        
        # For the merged-foreground, which is reported instead of class-0:
        self.DDlistPerSubepForegrRpRnTpTn = []
        self.DDlistPerSubepForegrMeanAccSensSpecDsc = [] # NOTE: May contain N0T-APPLICABLE=self.NA_PATTERN elements, eg when class not present!
    ...
    ##========================================================================##
    def DDupdateMonitorAccuraciesWithNewSubepochEntries(  self,
                                                        meanCostOfSubepoch,
                                                        perClassRpRnTpTnInSubep # Class X 4. The Real Pos, Real Neg, True Pos (pred), True Neg (pred).
                                                        ) :
        #---------- This first part takes care of the overall, multi-class mean accuracy: meanAccuracy = Num-Voxels-Predicted-Correct-Class / All-Voxels. 
        ...
        correctlyPredVoxelsInSubep = 0; # Predicted with the correct class.
        for class_i in range(self.DDnumberOfClasses) :
            correctlyPredVoxelsInSubep += perClassRpRnTpTnInSubep[class_i][2] # Add up the true positives per class (which are one vs all).
        self.DDcorrectlyPredVoxelsInEachSubep.append(correctlyPredVoxelsInSubep)
        numberOfAllSamples = perClassRpRnTpTnInSubep[0,0] + perClassRpRnTpTnInSubep[0,1] #RealPos + RealNeg wrt any class (eg backgr)
        self.DDnumberOfAllSamplesOfEachSubep.append(numberOfAllSamples)
        meanAccuracyOfSubepoch = self.NA_PATTERN if numberOfAllSamples == 0 else correctlyPredVoxelsInSubep*1.0/numberOfAllSamples
        self.DDmeanEmpiricalAccuracyOfEachSubep.append(meanAccuracyOfSubepoch)
        
        #----------- Calculate accuracy over subepoch for each class_i, in a One-Vs-All fashion ------
        self.DDlistPerSubepPerClassRpRnTpTn.append(perClassRpRnTpTnInSubep)
        ...
        for class_i in range(self.DDnumberOfClasses) :
            ...
            listWithPerClassMeanAccSensSpecDscInSubep.append( [meanAccuracyClassVsAllOfSubep, meanAccuracyOnPosOfSubep, meanAccuracyOnNegOfSubep] )
        self.DDlistPerSubepPerClassMeanAccSensSpecDsc.append(listWithPerClassMeanAccSensSpecDscInSubep)
    ...
    ##====================================================##
    def DDreportAccuracyForLastSubepoch(self) :
        trainOrValString = "TRAINING" if self.training0orValidation1 == 0 else "VALIDATION"
        numberOfClasses = self.DDnumberOfClasses
        currSubep = self.numberOfSubepochsForWhichUpdated - 1
        logStr = trainOrValString + ": Epoch #" + str(self.epoch) + ", Subepoch #" + str(currSubep)
        
        self.log.print3( "+++++++++++++++++++++++ Reporting Accuracy over whole subepoch For Domain Discriminator+++++++++++++++++++++++" )
        self.log.print3( logStr + ", Overall:\t mean accuracy:   \t" + strFl4fNA(self.DDmeanEmpiricalAccuracyOfEachSubep[currSubep], self.NA_PATTERN) +\
                        "\t=> Correctly-Classified-Voxels/All-Predicted-Voxels = " + str(self.DDcorrectlyPredVoxelsInEachSubep[currSubep]) + "/" + str(self.DDnumberOfAllSamplesOfEachSubep[currSubep]) )
        if self.training0orValidation1 == 0 : # During training, also report the mean value of the Cost Function:
            self.log.print3( logStr + ", Overall:\t mean cost:      \t" + strFl5fNA(self.meanCostOfEachSubep[currSubep], self.NA_PATTERN) ) 
        ...
        for class_i in range(self.DDnumberOfClasses) :
            ...
            [meanAccClassOfSubep,
            meanAccOnPosOfSubep,
            meanAccOnNegOfSubep ] = self.DDlistPerSubepPerClassMeanAccSensSpecDsc[currSubep][class_i]
            
            [numOfRpInSubep,
            numOfRnInSubep,
            numOfTpInSubep,
            numOfTnInSubep] = self.DDlistPerSubepPerClassRpRnTpTn[currSubep][class_i]
    ...
    def reportMeanAccyracyOfEpoch(self) :
        ...
        DDmeanEmpiricalAccOfEp = getMeanOfListExclNA(self.DDmeanEmpiricalAccuracyOfEachSubep, self.NA_PATTERN)
        ...
        self.log.print3( logStr + ", !!!!!Overall:\t Domain Discriminator mean accuracy of epoch:\t" + strFl4fNA(DDmeanEmpiricalAccOfEp, self.NA_PATTERN)+"\t=> Correctly-Classified-Voxels/All-Predicted-Voxels")
