...
from deepmedicUDA.frontEnd.configParsing.utils import getAbsPathEvenIfRelativeIsGiven, parseAbsFileLinesInList, parseFileLinesInList, check_and_adjust_path_to_ckpt
from deepmedicUDA.dataManagement import samplingType
...
class TrainSessionParameters(object) :
    ...
    def __init__(self,
        ...
        TDlistOfAListPerChannelWithFilepathsOfAllCasesTrain = [parseAbsFileLinesInList(getAbsPathEvenIfRelativeIsGiven(TDchannelConfPath, abs_path_to_cfg)) for TDchannelConfPath in cfg[cfg.TDCHANNELS_TR]]
        ...
        self.TDchanelsFilePathsTrain = [ list(item) for item in zip(*tuple(TDlistOfAListPerChannelWithFilepathsOfAllCasesTrain))]

        self.gtLabelsFilepathsTrain = parseAbsFileLinesInList( getAbsPathEvenIfRelativeIsGiven(cfg[cfg.GT_LABELS_TR], abs_path_to_cfg) )
        self.TDgtLabelsFilepathsTrain = parseAbsFileLinesInList( getAbsPathEvenIfRelativeIsGiven(cfg[cfg.TDGT_LABELS_TR], abs_path_to_cfg) )
        
        #[Optionals]
        #~~~~~~~~~Sampling~~~~~~~
        self.providedRoiMasksTrain = True if cfg[cfg.ROI_MASKS_TR] else False
        self.DDprovidedRoiMasksTrain = True if cfg[cfg.DDROI_MASKS_TR] else False
        ...
        self.DDroiMasksFilepathsTrain = parseAbsFileLinesInList( getAbsPathEvenIfRelativeIsGiven(cfg[cfg.DDROI_MASKS_TR], abs_path_to_cfg) ) if self.DDprovidedRoiMasksTrain else []
        ...
        self.TDsamplingTypeInstance = samplingType.SamplingType(self.log, TD_SAMPLING_TYPE, DDnum_classes)
        
        numberOfCategoriesOfSamples = self.TDsamplingTypeInstance.getNumberOfCategoriesToSample()
        
        self.TDsamplingTypeInstance.setPercentOfSamplesPerCategoryToSample( [1.0/numberOfCategoriesOfSamples]*numberOfCategoriesOfSamples )
        ##==========================================================##   
        ...
        if self.performValidationOnSamplesThroughoutTraining or self.performFullInferenceOnValidationImagesEveryFewEpochs :
            if cfg[cfg.CHANNELS_VAL] :
                ...
                TDlistOfAListPerChannelWithFilepathsOfAllCasesVal = [parseAbsFileLinesInList(getAbsPathEvenIfRelativeIsGiven(channelConfPath, abs_path_to_cfg)) for channelConfPath in cfg[cfg.TDCHANNELS_VAL]]
                self.channelsFilepathsVal = [ list(item) for item in zip(*tuple(listOfAListPerChannelWithFilepathsOfAllCasesVal)) ]
                self.TDchannelsFilePathsVal = [ list(item) for item in zip(*tuple(TDlistOfAListPerChannelWithFilepathsOfAllCasesVal)) ]
            else :
                self.errReqChannsVal()
                
        else :
            self.channelsFilepathsVal = []
            self.TDchannelsFilepathsVal = []
        if self.performValidationOnSamplesThroughoutTraining :
            ...
            self.TDgtLabelsFilepathsVal = parseAbsFileLinesInList( getAbsPathEvenIfRelativeIsGiven(cfg[cfg.TDGT_LABELS_VAL], abs_path_to_cfg) ) if cfg[cfg.TDGT_LABELS_VAL] is not None else self.errorReqGtLabelsVal()
            ...
            self.TDgtLabelsFilepathsVal = parseAbsFileLinesInList( getAbsPathEvenIfRelativeIsGiven(cfg[cfg.TDGT_LABELS_VAL], abs_path_to_cfg) ) if cfg[cfg.TDGT_LABELS_VAL] is not None else []
        else : # Dont perform either of the two validations.
            self.gtLabelsFilepathsVal = []
            self.TDgtLabelsFilepathsVal = []
        ...
        self.DDprovidedRoiMasksVal = True if cfg[cfg.DDROI_MASKS_VAL] is not None else False
        ...
        self.DDroiMasksFilepathsVal = parseAbsFileLinesInList( getAbsPathEvenIfRelativeIsGiven(cfg[cfg.DDROI_MASKS_VAL], abs_path_to_cfg) ) if self.DDprovidedRoiMasksVal else []
    ...
    def get_args_for_train_routine(self) :
        ...
        args = [self.log,
                ...
                self.TDchanelsFilePathsTrain, ##!!!!!!!!!!!!!!!ATTENTION!!!!!!!!
                self.channelsFilepathsVal,
                self.TDchannelsFilepathsVal, ##!!!!!!!!!!!!!Is it needed???????????/
                
                self.gtLabelsFilepathsTrain,
                self.TDgtLabelsFilepathsTrain,
                self.providedGtVal,
                self.gtLabelsFilepathsVal,
                self.TDgtLabelsFilepathsVal,
