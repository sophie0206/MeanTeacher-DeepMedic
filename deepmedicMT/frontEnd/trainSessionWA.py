    ...
    def run_session(self, *args):
        ...
        with tf.Session( graph=graphTf, config=tf.ConfigProto(log_device_placement=False, device_count={'CPU':999, 'GPU':99}) ) as sessionTf:
            ...
            if file_to_load_params_from is not None: # Load params
                ...
                ##============================================Added2019.10.05================================================##
                self._log.print3("\n===============================MT-WA Method=================================\n")
                reader = tf.train.NewCheckpointReader(chkpt_fname)# Get variables from pre-trained networks
                ...
                for v in collection_vars_net:
                    if "ExponentialMovingAverage" in v.name:
                        numOfExponentials += 1
                l = len(collection_vars_net) - numOfExponentials
                rdmInit_vars = collection_vars_net[l-11:] + collection_vars_tch_net ## softmax layer parameters.and also random initialize the plus units weights.
                ...
                for key in var:
                    ...
                    ##############################################################################################
                    if (key == 'net/W_25'):  #########################Not Sure!!!!!!!!!!! And delete the corresponding W25 in stroke model variables.
                        W25 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-11][:250, :, :, :, :], W25))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-11][:250, :, :, :, :], W25))
                    if (key == 'net/gBn_23'):
                        gBn23 = reader.get_tensor(key)
                        self._log.print3(str(collection_vars_tch_net[l-10].name) + str(collection_vars_net[l-10].name))
                        assign_ops.append(tf.assign(collection_vars_net[l-9][:250], gBn23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-9][:250], gBn23))
                    if (key == 'net/bBn_23'):
                        bBn23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-8][:250], bBn23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-8][:250], bBn23))
                    if (key == 'net/muBnsForRollingAverage_23'):
                        mu23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-7][:, :250], mu23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-7][:, :250], mu23))
                    if (key == 'net/varBnsForRollingAverage_23'):
                        var23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-6][:, :250], var23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-6][:, :250], var23))
                    if (key == 'net/sharedNewMu_B_23'):
                        sMu23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-5][:250], sMu23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-5][:250], sMu23))
                    if (key == 'net/sharedNewVar_B_23'):
                        sVar23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-4][:250], sVar23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-4][:250], sVar23))
                    if (key == 'net/aPrelu_23'):
                        aP23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[l-3][:250], aP23))
                        #assign_ops.append(tf.assign(collection_vars_tch_net[l-3][:250], aP23))
                ...
                while i < (len(collection_vars_net) - 6):
                    ...
                    if v.name == 'net/W:0':
                        assign_ops.append(tf.assign(v, W0))
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], W0))
                        ...
                        i = i - 1
                    if v.name == 'net/b:0':
                        assign_ops.append(tf.assign(v, b0))
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], b0))
                    ...
                    if v.name == 'net/W_8:0':
                        assign_ops.append(tf.assign(v, W8))
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], W8))
                        ...
                        i = i - 1
                    if v.name == 'net/b_1:0':
                        assign_ops.append(tf.assign(v, b1))
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], b1))
                    ...
                    if v.name == 'net/W_16:0':
                        assign_ops.append(tf.assign(v, W16))
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], W16))
                        self._log.print3(str(collection_vars_net[i].shape))
                        del collection_vars_net[i]
                        del collection_vars_tch_net[i]
                        i = i - 1
                    if v.name == 'net/b_2:0':
                        assign_ops.append(tf.assign(v, b2))
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], b2))
                ...
                reset_trainer = True
                saver_net = tf.train.Saver( var_list = collection_vars_net[:-numOfExponentials-11] ) # Used to load the net's parameters.
                ...
                try:
                    ...
                    ##===================================================================================================##
                    for i in range(l-6):
                        for j in range(numOfExponentials):
                            if collection_vars_net[i].name[:-2] == collection_vars_net[l-6+j].name[:-27]:
                                #self._log.print3(str(collection_vars_net[i].name[:-2]) + " " + str(collection_vars_net[l-6+j].name))
                                assign_ops.append(tf.assign(collection_vars_net[l-6+j], collection_vars_net[i]))
                                
                    for i in range(len(collection_vars_net)):
                        assign_ops.append(tf.assign(collection_vars_tch_net[i], collection_vars_net[i]))
                        
                    sessionTf.run(assign_ops)
                    self._log.print3("Finished assign teacher model parameters. ")
                    ##==================================================================================================##
