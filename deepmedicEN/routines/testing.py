...
from deepmedicEN.dataManagement.sampling import load_imgs_of_single_case
from deepmedicEN.dataManagement.sampling import getCoordsOfAllSegmentsOfAnImage
from deepmedicEN.dataManagement.sampling import extractDataOfSegmentsUsingSampledSliceCoords
from deepmedicEN.image.io import savePredImgToNiiWithOriginalHdr, saveFmImgToNiiWithOriginalHdr, save4DImgWithAllFmsToNiiWithOriginalHdr
from deepmedicEN.image.processing import unpadCnnOutputs

from deepmedicEN.neuralnet.pathwayTypes import PathwayTypes as pt
from deepmedicEN.logging.utils import strListFl4fNA, getMeanPerColOf2dListExclNA
...
def performInferenceOnWholeVolumes(
    ...
    for image_i in range(total_number_of_images) :
        ...
        for batch_i in range(number_of_batches) : #batch_size = how many image parts in one batch. Has to be the same with the batch_size it was created with. This is no problem for testing. Could do all at once, or just 1 image part at time.
            ...
            for path_i in range(len(channsOfSegmentsPerPath[1:])-1) :
