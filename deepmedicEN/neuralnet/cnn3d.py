...
from deepmedicEN.neuralnet.pathwayTypes import PathwayTypes as pt
from deepmedicEN.neuralnet.pathways import NormalPathway, SubsampledPathway, FcPathway, DDPathway, getMiddlePartOfFms
from deepmedicEN.neuralnet.layers import SoftmaxLayer, ConvLayer
...
class Cnn3d(object):
    ...
    def setup_ops_n_feeds_to_train(self, log, total_cost, updates_of_params_wrt_total_cost) :
        ...
        self._ops_main['train']['list_rp_rn_tp_tn'] = self.finalTargetLayer.getRpRnTpTnForTrain0OrVal1(y_gt, self.batchSize_seg["train"], 0)
        ##===========================================================##
        self._ops_main['train']['DDlist_rp_rn_tp_tn'] = self.DDfinalTargetLayer.getRpRnTpTnForTrain0OrVal1DD(yDD_gt, 0)
        ##===========================================================##
    ...
    def setup_ops_n_feeds_to_val(self, log) :
        ...
        self._ops_main['val'] = {}
        self._ops_main['val']['list_rp_rn_tp_tn'] = self.finalTargetLayer.getRpRnTpTnForTrain0OrVal1(y_gt, self.batchSize_seg["val"], 1)
    ...
    ##============================================================================================##
    def makeDDPathwayInputs(self, NormalPathwayInput, str_train_val_test, probabilityMaps):

        shapeToCrop =  self.getFcPathway().getLayer(-1).outputShape[str_train_val_test][2:]
        croppedInput = getMiddlePartOfFms(NormalPathwayInput, shapeToCrop)
      

        for i in range(1):
            for j in range(2):
                multiplied1 = tf.multiply(croppedInput[:, i, :, :, :], probabilityMaps[:, j, :, :, :])
                multiplied1Reshaped = tf.reshape(multiplied1, shape=[self.batchSize_adv[str_train_val_test], 1] + shapeToCrop )
                
                multiplied2 = tf.multiply((1 - croppedInput[:, i, :, :, :]), probabilityMaps[:, j, :, :, :])
                multiplied2Reshaped = tf.reshape(multiplied2, shape=[self.batchSize_adv[str_train_val_test], 1] + shapeToCrop)
                
                concatenated = tf.concat([multiplied1Reshaped, multiplied2Reshaped], axis=1)

                if (i == 0 and j == 0):            
                    DDInputImage = concatenated
                else:
                    DDInputImage = tf.concat([DDInputImage, concatenated], axis=1)

        return DDInputImage, shapeToCrop
     ##==============================================================================================##  
    ...
    def make_cnn_model( self,
        ...
        self.batchSize_adv = {"train": batch_size_train, "val": batch_size_validation, "test": batch_size_testing}
        ##============================================================================================#
        # == Others ==
...
self.batchSize_seg,
        ...
        NormalPathwayInputTrain = thisPathway.getLayer(0).input["train"]
        NormalPathwayInputVal = thisPathway.getLayer(0).input["val"]
        NormalPathwayInputTest = thisPathway.getLayer(0).input["test"]
        ##======================================================================================================================================================##
...
self.batchSize_seg,
        ...
        probabilityMapsOTr = self.finalTargetLayer.p_y_given_x_train
        probabilityMapsTr = tf.concat([probabilityMapsOTr[:, :3, :, :, :],probabilityMapsOTr[:, 4:, :, :, :]], axis=1)
        
        probabilityMapsOVal = self.finalTargetLayer.p_y_given_x_val
        probabilityMapsVal = tf.concat([probabilityMapsOVal[:, :3, :, :, :],probabilityMapsOVal[:, 4:, :, :, :]], axis=1)
        
        probabilityMapsOTe = self.finalTargetLayer.p_y_given_x_test
        probabilityMapsTe = tf.concat([probabilityMapsOTe[:, :3, :, :, :],probabilityMapsOTe[:, 4:, :, :, :]], axis=1)
...
##=============================================================================================================##
        ...
        inputToPathwayTrain, shapeTr  = self.makeDDPathwayInputs(NormalPathwayInputTrain, "train", probabilityMapsTr)
        inputToPathwayVal, shapeVal = self.makeDDPathwayInputs(NormalPathwayInputVal, "val", probabilityMapsVal)
        inputToPathwayTest, shapeTe = self.makeDDPathwayInputs(NormalPathwayInputTest, "test", probabilityMapsTe)
        
        ##================================================================#
        ##DDbatchSize, imagePartDimensionsTrain is the size of image segment.
        
        inputToPathwayShapeTrain= [self.batchSize_adv["train"]] + [4] + shapeTr
        inputToPathwayShapeVal = [self.batchSize_adv["val"]] + [4] + shapeVal
        inputToPathwayShapeTest = [self.batchSize_adv["test"]] + [4] + shapeTe
        
        log.print3("\n\n\n#=======================DD Pathway Input Shape:" + str(inputToPathwayShapeTrain))
