...
import deepmedicEN.neuralnet.optimizers as optimizers_dm
import deepmedicEN.neuralnet.cost_functions as cfs
...
class Trainer(object):
    ...
    def _setup_costs(self, log):
        ...
        ##==============================================================##
        yDD_gt = self._net._DDoutput_gt_tensor_feeds['train']['y_gt']
        ##==============================================================##
        if "xentr" in self._losses_and_weights and self._losses_and_weights["xentr"] is not None:
            ...
            cost += self._losses_and_weights["xentr"] * cfs.x_entr( self._net.finalTargetLayer.p_y_given_x_train[:self._net.batchSize_seg["train"],:,:,:,:], y_gt, w_per_cl_vec )
           ##============================================================================================================================##  
            w_per_cl_vecDD = self._compute_w_per_class_vector_for_xentr( self._net.DDnum_classes, yDD_gt) #num_classes not the same either!!!!
            DDcost += self._losses_and_weights["xentr"] * cfs.DDx_entr(self._net.DDfinalTargetLayer.p_y_given_x_train, yDD_gt, w_per_cl_vecDD)
            ##!!!!!!!!!!!!Two different final target layers.
        ...
        if "iou" in self._losses_and_weights and self._losses_and_weights["iou"] is not None:
            ...
            cost += self._losses_and_weights["iou"] * cfs.iou( self._net.finalTargetLayer.p_y_given_x_train[:self._net.batchSize_seg["train"],:,:,:,:], y_gt )
            DDcost += self._losses_and_weights["iou"] * cfs.iou( self._net.DDfinalTargetLayer.p_y_given_x_train, yDD_gt )
        ...
        if "dsc" in self._losses_and_weights and self._losses_and_weights["dsc"] is not None:
            ...
            cost += self._losses_and_weights["dsc"] * cfs.dsc( self._net.finalTargetLayer.p_y_given_x_train[:self._net.batchSize_seg["train"],:,:,:,:], y_gt )
            DDcost += self._losses_and_weights["dsc"] * cfs.dsc( self._net.DDfinalTargetLayer.p_y_given_x_train, yDD_gt )

        ## It calculated all layers from all pathways here, including the DD, so I don't need extra code.
        ...
        ##=====================Add alpha here as the weight parameter in total cost function======##
        alpha = cfs.getAlphaOfThisEpoch(self._num_epochs_trained_tfv)
        self._total_cost = cost - alpha * DDcost
        ##========================================================================##
