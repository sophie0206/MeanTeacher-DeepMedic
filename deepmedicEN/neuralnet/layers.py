...
from deepmedicEN.neuralnet.ops import applyDropout, makeBiasParamsAndApplyToFms, makeDDBiasParamsAndApplyToFms, applyRelu, applyPrelu, applyElu, applySelu, pool3dMirrorPad
from deepmedicEN.neuralnet.ops import applyBn, createAndInitializeWeightsTensor, convolveWithGivenWeightMatrix, applySoftmaxToFmAndReturnProbYandPredY
...
class ConvLayer(Block):
    ...
    def _get_L1_cost(self) : #Called for L1 weights regularisation
        return tf.reduce_sum(tf.abs(self._W))
    def _get_L2_cost(self) : #Called for L2 weights regularisation
        return tf.reduce_sum(self._W ** 2)
    

    #==========================My added part: Make a convlayer for domain discriminator===========#
    def makeDDLayer(self,
                DDrng,
                inputToDDLayerTrain,
                inputToDDLayerVal,
                inputToDDLayerTest,
                inputToDDLayerShapeTrain,
                inputToDDLayerShapeVal,
                inputToDDLayerShapeTest,
                DDfilterShape,
                DDpoolingParameters, # Can be []
                DDconvWInitMethod,
                DDuseBnFlag, # Must be true to do BN. Used to not allow doing BN on first layers straight on image, even if rollingAvForBnOverThayManyBatches > 0.
                DDmovingAvForBnOverXBatches, #If this is <= 0, we are not using BatchNormalization, even if above is True.
                DDactivationFunc="relu",
                DDdropoutRate=0.0):

        self._setBlocksInputAttributes(inputToDDLayerTrain, inputToDDLayerVal, inputToDDLayerTest, inputToDDLayerShapeTrain, inputToDDLayerShapeVal, inputToDDLayerShapeTest)
        self._setBlocksArchitectureAttributes(DDfilterShape, DDpoolingParameters)
        
        # Apply all the straightforward operations on the input, such as BN, activation function, dropout, pooling        
        (DDinputToConvTrain, DDinputToConvVal, DDinputToConvTest,
        DDinputToConvShapeTrain, DDinputToConvShapeVal, DDinputToConvShapeTest) = self._processInputWithBnNonLinearityDropoutPooling(DDrng,
                                                                                        inputToDDLayerTrain,
                                                                                        inputToDDLayerVal,
                                                                                        inputToDDLayerTest,
                                                                                        inputToDDLayerShapeTrain,
                                                                                        ...
                                                                                        DDactivationFunc,
                                                                                        DDdropoutRate)
        print ("Check this layer input Shape" + str(inputToDDLayerShapeVal))
        
        tupleWithOuputAndShapeTrValTest = self._createWeightsTensorAndConvolve(DDrng, DDfilterShape, DDconvWInitMethod, 
                                                                                DDinputToConvTrain, DDinputToConvVal, DDinputToConvTest,
                                                                                DDinputToConvShapeTrain, DDinputToConvShapeVal, DDinputToConvShapeTest)
...
class FcLayer(Block):
    ...
    def makeFcLayer(self, inputToLayerTrain, inputToLayerVal, inputToLayerTest, 
                    Trainshape, Valshape, Testshape, rng):
        #First to Flatten
        inputToLayerTrainReshaped = tf.transpose(inputToLayerTrain, perm=[0,2,3,4,1]) # [batchSize, r, c, z, #classes), the classes stay as the last dimension.
        inputToLayerTrainFlattened = tf.reshape(inputToLayerTrainReshaped, shape=[-1]) 
        firstdim = Trainshape[4]*Trainshape[2]*Trainshape[3]*Trainshape[1]
        OutClasses = Trainshape[1]
        inputToLayerTrain2d = tf.reshape(inputToLayerTrainFlattened, shape=[Trainshape[0], firstdim, 1])
        
        filterShapeTrain = [Trainshape[0], OutClasses, firstdim]
        WTrain = createAndInitializeWeightsTensor(filterShapeTrain, "Fc", rng)
       
        yTrain = tf.matmul(WTrain, inputToLayerTrain2d)
        yTrain = tf.reshape(yTrain, shape=[Trainshape[0], OutClasses])
        yTrainShape = [Trainshape[0], OutClasses]

##===========================================================
        inputToLayerValReshaped = tf.transpose(inputToLayerVal, perm=[0,2,3,4,1]) # [batchSize, r, c, z, #classes), the classes stay as the last dimension.
        inputToLayerValFlattened = tf.reshape(inputToLayerValReshaped, shape=[-1]) 
        firstdim = Valshape[4]*Valshape[2]*Valshape[3]*Valshape[1]
        OutClasses = Valshape[1]
        inputToLayerVal2d = tf.reshape(inputToLayerValFlattened, shape=[Valshape[0], firstdim, 1])
        
        filterShapeVal = [Valshape[0], OutClasses, firstdim]
        WVal = createAndInitializeWeightsTensor(filterShapeVal, "Fc", rng)
        yVal = tf.matmul(WVal, inputToLayerVal2d)

        yVal = tf.reshape(yVal, shape=[Valshape[0], OutClasses])
        yValShape = [Valshape[0], OutClasses]
##=============================================================
        inputToLayerTestReshaped = tf.transpose(inputToLayerTest, perm=[0,2,3,4,1]) # [batchSize, r, c, z, #classes), the classes stay as the last dimension.
        inputToLayerTestFlattened = tf.reshape(inputToLayerTestReshaped, shape=[-1]) 
        firstdim = Testshape[4]*Testshape[2]*Testshape[3]*Testshape[1]
        OutClasses = Testshape[1]
        inputToLayerTest2d = tf.reshape(inputToLayerTestFlattened, shape=[Testshape[0], firstdim, 1])
        
        filterShapeTest = [Testshape[0], OutClasses, firstdim]
        WTest = createAndInitializeWeightsTensor(filterShapeTest, "Fc", rng)
        yTest = tf.matmul(WTest, inputToLayerTest2d)

        yTest = tf.reshape(yTest, shape=[Testshape[0], OutClasses])
        yTestShape = [Testshape[0], OutClasses]

        self._W = WTrain
        self.params = [self._W] + self.params


        self._setBlocksOutputAttributes(yTrain, yVal, yTest, yTrainShape, yValShape, yTestShape)
...
class SoftmaxLayer(TargetLayer):
    ...
    def makeDDLayer(  self,
        ...
        self._numberOfOutputClasses = layerConnected.outputShape["train"][1]
        ...
        biasedInputToSoftmaxTest) = makeDDBiasParamsAndApplyToFms( self.input["train"], self.input["val"], self.input["test"], self._numberOfOutputClasses )
        self.params = self.params + [self._b]
        
        # ============ Softmax ==============
        self.p_y_given_x_train = tf.nn.softmax(biasedInputToSoftmaxTrain/softmaxTemperature, axis=-1)
        self.y_pred_train = tf.argmax(self.p_y_given_x_train, axis=1) #Result: batchSize, 

        self.p_y_given_x_val = tf.nn.softmax(biasedInputToSoftmaxVal/softmaxTemperature, axis=-1)
        self.y_pred_val = tf.argmax(self.p_y_given_x_val, axis=1) #Result: batchSize, 
        
        self.p_y_given_x_test = tf.nn.softmax(biasedInputToSoftmaxTest/softmaxTemperature, axis=-1)
        self.y_pred_test = tf.argmax(self.p_y_given_x_test, axis=1) #Result: batchSize, 
    ...
    def makeLayer(  self,
        ...
        ##=============================Labeled and Unlabeled together do classification=============================================##
        outputFcTrain, outputFcVal, outputFcTest = layerConnected.output["train"], layerConnected.output["val"], layerConnected.output["test"]

        shapeTr, shapeVal, shapeTe = layerConnected.outputShape["train"], layerConnected.outputShape["val"], layerConnected.outputShape["test"]
        outputBsegTrainShape = [batchSize_seg["train"] * 2] + shapeTr[1:]
        outputBsegValShape = [batchSize_seg["val"] * 2] + shapeVal[1:]
        outputBsegTestShape = [batchSize_seg["test"] * 2] + shapeTe[1:]

        
        self._setBlocksInputAttributes(outputFcTrain, outputFcVal, outputFcTest, 
    ...
    def get_DDoutput_gt_tensor_feed(self):
        # Input. Dimensions of y labels: [batchSize, r, c, z]
        y_gt_train = tf.placeholder(dtype="int32", shape=[None], name="y_train")
        y_gt_val = tf.placeholder(dtype="int32", shape=[None], name="y_val")
        return (y_gt_train, y_gt_val)
    ...
    def getRpRnTpTnForTrain0OrVal1(self, y, batchSize_seg, training0OrValidation1):
        ...
        # param y: y = T.itensor4('y'). Dimensions [batchSize, r, c, z]
        labeled_index = batchSize_seg
        yPredToUse = self.y_pred_train[:labeled_index, :, :, :] if  training0OrValidation1 == 0 else self.y_pred_val[:labeled_index, :, :, :]
    ...
    def getRpRnTpTnForTrain0OrVal1DD(self, y, training0OrValidation1):
        # The returned list has (numberOfClasses)x4 integers: >numberOfRealPositives, numberOfRealNegatives, numberOfTruePredictedPositives, numberOfTruePredictedNegatives< for each class (incl background).
        # Order in the list is the natural order of the classes (ie class-0 RP,RN,TPP,TPN, class-1 RP,RN,TPP,TPN, class-2 RP,RN,TPP,TPN ...)
        # param y: y = T.itensor4('y'). Dimensions [batchSize, r, c, z]
