...
from deepmedicEN.image.io import loadVolume
from deepmedicEN.image.processing import reflectImageArrayIfNeeded, calculateTheZeroIntensityOf3dImage, padCnnInputs
...
def getSampledDataAndLabelsForSubepoch(log,
    ...
    for index_for_vector_with_images_on_gpu in range(0, numOfSubjectsLoadingThisSubepochForSampling) :
        ...
        allSubsampledChannelsOfPatientInNpArray,  #a nparray(channels,dim0,dim1,dim2)h
...
def getTDSampledDataAndLabelsForSubepoch(log,
    ...
    log.print3(":=:=:=:=:=:=:=:=: Finished extracting Segments from the TARGET DOMAIN images for next " + training_or_validation_str + ". :=:=:=:=:=:=:=:=:")
...
def extractDataOfSegmentsUsingSampledSliceCoords(cnn3d,
    ...
    for segment_i in range(numberOfSegmentsToExtract) :
        ...
        #Subsampled pathways
        for pathway_i in range(len(cnn3d.pathways)) : # Except Normal 1st, cause that was done already.
            if cnn3d.pathways[pathway_i].pType() == pt.FC or cnn3d.pathways[pathway_i].pType() == pt.NORM or cnn3d.pathways[pathway_i].pType() == pt.DD:
                continue
