...
from deepmedicO.dataManagement.sampling import getSampledDataAndLabelsForSubepochRP3
...
def doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(log,
                                                                ...
                                                                second_channsOfSegmentsForSubepPerPathway) :
    """
    ...
    for batch_i in range(number_of_batches):
        ...
        if train_or_val=="train" :
            ...
            single_batch_size = cnn3d.batchSize["train"] // 2
            index_to_data_for_batch_min = batch_i * single_batch_size
            index_to_data_for_batch_max = (batch_i + 1) * single_batch_size

            thisBatchAlldata = np.concatenate((channsOfSegmentsForSubepPerPathway[0][index_to_data_for_batch_min : index_to_data_for_batch_max], second_channsOfSegmentsForSubepPerPathway[0][index_to_data_for_batch_min: index_to_data_for_batch_max]), axis=0)

            feeds = cnn3d.get_main_feeds('train')
            feeds_dict = { feeds['x'] : thisBatchAlldata }
            for subsPath_i in range(cnn3d.numSubsPaths) :
                sub_alldata = np.concatenate((channsOfSegmentsForSubepPerPathway[ subsPath_i+1 ][ index_to_data_for_batch_min : index_to_data_for_batch_max ], second_channsOfSegmentsForSubepPerPathway[ subsPath_i+1 ][ index_to_data_for_batch_min : index_to_data_for_batch_max ]), axis=0)
                feeds_dict.update( { feeds['x_sub_'+str(subsPath_i)]: sub_alldata } )
        ...
        for subepoch in range(number_of_subepochs): #per subepoch I randomly load some images in the gpu. Random order.
            ...
            if performValidationOnSamplesDuringTrainingProcessBool :
                if boolItIsTheVeryFirstSubepochOfThisProcess :
                    ...
                    labelsForCentralOfSegmentsForSubepVal, nothing] = getSampledDataAndLabelsForSubepochRP3(log,
                                                                        "val",
                                                                        run_input_checks,
                                                                        cnn3dWrapper,
                                                                        maxNumSubjectsLoadedPerSubepoch,
                    ...
                    [channsOfSegmentsForSubepPerPathwayVal,
                    labelsForCentralOfSegmentsForSubepVal, nothing] = parallelJobToGetDataForNextValidation() #fromParallelProcessing that had started from last loop when it was submitted.
                    
                # Below is computed with number of extracted samples, in case I dont manage to extract as many as I wanted initially.
                numberOfBatchesValidation = 2 * len(channsOfSegmentsForSubepPerPathwayVal[0]) // cnn3d.batchSize["val"]
                ...
                parallelJobToGetDataForNextTraining = job_server.submit(getSampledDataAndLabelsForSubepochRP3, #local function to call and execute in parallel.
            ...
            if (not performValidationOnSamplesDuringTrainingProcessBool) and boolItIsTheVeryFirstSubepochOfThisProcess :                    
                ...
                labelsForCentralOfSegmentsForSubepTrain, second_channsOfSegmentsForSubepPerPathwayTrain] = getSampledDataAndLabelsForSubepochRP3(log,
                                                                        "train",
                                                                        run_input_checks,
                                                                        cnn3dWrapper,
                                                                        maxNumSubjectsLoadedPerSubepoch,
                ...
                [channsOfSegmentsForSubepPerPathwayTrain,
                labelsForCentralOfSegmentsForSubepTrain, second_channsOfSegmentsForSubepPerPathwayTrain] = parallelJobToGetDataForNextTraining() #fromParallelProcessing that had started from last loop when it was submitted.
            
            numberOfBatchesTraining = (2 * len(channsOfSegmentsForSubepPerPathwayTrain[0])) // cnn3d.batchSize["train"] #Computed with number of extracted samples, in case I dont manage to extract as many as I wanted initially.
            ...
            if performValidationOnSamplesDuringTrainingProcessBool :
                ...
                parallelJobToGetDataForNextValidation = job_server.submit(getSampledDataAndLabelsForSubepochRP3, #local function to call and execute in parallel.
            ...
            doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(log,
                                                                        ...
                                                                        second_channsOfSegmentsForSubepPerPathwayTrain)
