...
def mixup_input(X, Y, cnn3d, lam, log):
    ...
    for i in range(0, cnn3d.batchSize["train"]*2):
        y = Y[i]
    ...
    mixed_X = [np.ones(X[0].shape)] * cnn3d.batchSize["train"]

    
    for j in range(cnn3d.batchSize["train"]):
...
def doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(log,
    ...
    number_of_batches = number_of_batches // 2
    ...
    for batch_i in range(number_of_batches):
        ...
        if train_or_val=="train" :
            ...
            ##==================================================================================##
            index_to_data_for_batch_min = batch_i * cnn3d.batchSize["train"]  * 2
            index_to_data_for_batch_max = (batch_i + 1) * cnn3d.batchSize["train"] * 2
            alpha = 0.4
            lam = np.random.beta(alpha, alpha)
            
            this_batch_y = labelsForCentralOfSegmentsForSubep[ index_to_data_for_batch_min : index_to_data_for_batch_max ]
             

            mixup_X_NP = mixup_input(channsOfSegmentsForSubepPerPathway[0][ index_to_data_for_batch_min : index_to_data_for_batch_max ], this_batch_y , cnn3d, lam, log)
            #log.print3(str(len(mixup_X_NP)) + str(mixup_X_NP[1].shape))
            ##==================================================================================##
            feeds = cnn3d.get_main_feeds('train')
            feeds_dict = { feeds['x'] : mixup_X_NP }

            for subsPath_i in range(cnn3d.numSubsPaths) :
                mixup_X_subP = mixup_input(channsOfSegmentsForSubepPerPathway[subsPath_i+1][ index_to_data_for_batch_min : index_to_data_for_batch_max ], this_batch_y , cnn3d, lam, log)
                #log.print3(str(len(mixup_X_subP))+ str(mixup_X_subP[0].shape))
                feeds_dict.update( { feeds['x_sub_'+str(subsPath_i)]: mixup_X_subP } )
            
            sorted_Y = sorted(this_batch_y, key = lambda y: len(y[y>0]))
            ##=======================================================================##
            mixed_Y = [np.ones(this_batch_y[0].shape)] * cnn3d.batchSize["train"]
            for i in range(cnn3d.batchSize["train"]):
                mixed_Y[i] = lam * sorted_Y[i] + (1-lam) * sorted_Y[-i-1]
            ##========================================================================##
            feeds_dict.update( { feeds['y_gt'] : mixed_Y } )
