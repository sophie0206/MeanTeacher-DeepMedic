...
# mixup and original patches all feed into the model. combined.
...
from deepmedicO.neuralnet.wrappers import CnnWrapperForSampling
from deepmedicO.dataManagement.sampling import getSampledDataAndLabelsForSubepoch
from deepmedicO.routines.testing import performInferenceOnWholeVolumes
...
def mixup_input(X, Y, cnn3d, lam, log):
    # we should use the same lam every batch
    #log.print3(str(len(X[0])) + str(len(Y)))
    mapFromIndextoNum = dict()
    for i in range(0, cnn3d.batchSize["train"]):
        y = Y[i]
        mapFromIndextoNum[i] = len(y[y>0])

    sorted_maplist = sorted(mapFromIndextoNum.items(), key = lambda x: x[1])  
    
    #In this way, help mixed x to find corresponding y1 and y2 when computing mixedup loss.
    
    mixed_X = [np.ones(X[0].shape)] * (cnn3d.batchSize["train"] // 2)

    
    for j in range(cnn3d.batchSize["train"] // 2):
        minNumIndex = sorted_maplist[j][0]
        maxNumIndex = sorted_maplist[-j-1][0]
        mixed_X[j] = lam * X[minNumIndex] + (1-lam) * X[maxNumIndex]
        #log.print3(str(mixed_X[j].shape))
    return mixed_X
...
def doTrainOrValidationOnBatchesAndReturnMeanAccuraciesOfSubepoch(log,
    ...
    number_of_batches = int(number_of_batches // 1.5)
    ...
    for batch_i in range(number_of_batches):
        ...
        if train_or_val=="train" :
            ...
            ##==================================================================================##
            index_to_data_for_batch_min = int(batch_i * cnn3d.batchSize["train"] * 1.5)
            index_to_data_for_batch_max = int((batch_i + 1) * cnn3d.batchSize["train"] * 1.5)
            alpha = 0.4
            lam = np.random.beta(alpha, alpha)
            
            index_for_mixup = cnn3d.batchSize["train"] + index_to_data_for_batch_min
            this_batch_y = labelsForCentralOfSegmentsForSubep[ index_to_data_for_batch_min : index_for_mixup ]
             
            mixup_X_NP = mixup_input(channsOfSegmentsForSubepPerPathway[0][ index_to_data_for_batch_min : index_for_mixup], this_batch_y , cnn3d, lam, log)
            #log.print3(str(len([channsOfSegmentsForSubepPerPathway[0][ index_for_mixup:index_to_data_for_batch_max ]] + [mixup_X_NP])))
            ##==================================================================================##
            feeds = cnn3d.get_main_feeds('train')
            feeds_dict = { feeds['x'] : np.concatenate((channsOfSegmentsForSubepPerPathway[0][ index_for_mixup:index_to_data_for_batch_max ], mixup_X_NP), axis=0) }

            for subsPath_i in range(cnn3d.numSubsPaths) :
                mixup_X_subP = mixup_input(channsOfSegmentsForSubepPerPathway[subsPath_i+1][ index_to_data_for_batch_min : index_for_mixup ], this_batch_y , cnn3d, lam, log)
                #log.print3(str(len(mixup_X_subP))+ str(mixup_X_subP[0].shape))
                feeds_dict.update( { feeds['x_sub_'+str(subsPath_i)]: np.concatenate((channsOfSegmentsForSubepPerPathway[subsPath_i+1][ index_for_mixup:index_to_data_for_batch_max ], mixup_X_subP), axis=0) } )
            
            sorted_Y = sorted(this_batch_y, key = lambda y: len(y[y>0]))
            ##=======================================================================##
            mixed_Y = [np.ones(this_batch_y[0].shape)] * (cnn3d.batchSize["train"]//2)
            for i in range(cnn3d.batchSize["train"] // 2):
                mixed_Y[i] = lam * sorted_Y[i] + (1-lam) * sorted_Y[-i-1]
            ##========================================================================##
            #log.print3(str(lam))
            #log.print3(str(len([labelsForCentralOfSegmentsForSubep[ index_for_mixup:index_to_data_for_batch_max ]] + mixed_Y))) 
            feeds_dict.update( { feeds['y_gt'] : np.concatenate((labelsForCentralOfSegmentsForSubep[ index_for_mixup:index_to_data_for_batch_max ], mixed_Y), axis=0) } )
            #feeds_dict.update({feeds['lam']: lam})
            # Training step
...
def do_training(sessionTf,
                ...
                #-------Sampling Type---------
                samplingTypeInstanceTraining, # Instance of the deepmedicOO/samplingType.SamplingType class for training and validation
    ...
    tupleWithModulesToImportWhichAreUsedByTheJobFunctions = ( "from __future__ import absolute_import, print_function, division",
                "time", "numpy as np", "from deepmedicO.dataManagement.sampling import *" )
