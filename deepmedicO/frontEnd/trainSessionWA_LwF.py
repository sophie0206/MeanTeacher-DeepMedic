...
# Integrated Width-augmented Method with Learning without Forgetting!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
...
from deepmedicO.neuralnet.cnn3dWA_LwF import Cnn3d
from deepmedicO.neuralnet.trainerWA_LwF import Trainer
    ...
    def run_session(self, *args):
        ...
        with tf.Session( graph=graphTf, config=tf.ConfigProto(log_device_placement=False, device_count={'CPU':999, 'GPU':99}) ) as sessionTf:
            ...
            if file_to_load_params_from is not None: # Load params
                ...
                rdmInit_vars = collection_vars_net[-10:] ## get the last two, softmax layer parameters.and also random initialize the plus units weights.
                tf.variables_initializer(rdmInit_vars).run()
                
                l_i = 0
                r_i = 1
                self._log.print3(str(r_i) + " Channel training\n Width-augmented&Learning without Forgetting Method!")
                for key in var:
                    if key == 'net/W':
                    ...
                    if key == 'net/W_25':  #########################Not Sure!!!!!!!!!!! And delete the corresponding W25 in stroke model variables.
                        ...
                        sessionTf.run(tf.assign(collection_vars_net[-10][:250, :, :, :, :], W25))
                    if key == 'net/gBn_23':
                        gBn23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-9][:250], gBn23))
                    if key == 'net/bBn_23':
                        bBn23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-8][:250], bBn23))
                    if key == 'net/muBnsForRollingAverage_23':
                        mu23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-7][:, :250], mu23))
                    if key == 'net/varBnsForRollingAverage_23':
                        var23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-6][:, :250], var23))
                    if key == 'net/sharedNewMu_B_23':
                        sMu23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-5][:250], sMu23))
                    if key == 'net/sharedNewVar_B_23':
                        sVar23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-4][:250], sVar23))
                    if key == 'net/aPrelu_23':
                        aP23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-3][:250], aP23))
                    if key == 'net/W_26':
                        W26 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-2][:5, :250, :, :, :], W26))
                    if key == 'net/b_3':
                        b3 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-1][:5], b3))
                ...
                saver_net = tf.train.Saver( var_list=collection_vars_net[:-10] ) # Used to load the net's parameters.
