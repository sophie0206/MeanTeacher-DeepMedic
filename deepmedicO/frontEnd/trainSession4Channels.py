    ...
    def run_session(self, *args):
        ...
        with tf.Session( graph=graphTf, config=tf.ConfigProto(log_device_placement=False, device_count={'CPU':999, 'GPU':99}, gpu_options=tf.GPUOptions(allow_growth=True)) ) as sessionTf:
            ...
            if file_to_load_params_from is not None: # Load params
                ...
                collection_vars_net.pop(len(collection_vars_net) - 1) ## random initialize b_3, the bias in softmax layer
                
                tf.variables_initializer([collection_vars_net[-1]]).run()
                collection_vars_net.pop(len(collection_vars_net) - 1)## random initialize W_26, [5, 250, 1, 1, 1], weight of the last fc layer
