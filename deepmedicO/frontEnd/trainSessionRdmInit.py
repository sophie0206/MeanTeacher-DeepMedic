...
###Random initialize the first layer!
        ...
        with tf.Session( graph=graphTf, config=tf.ConfigProto(log_device_placement=False, device_count={'CPU':999, 'GPU':99}) ) as sessionTf:
            ...
            if file_to_load_params_from is not None: # Load params
                ...
                while i < (len(collection_vars_net) - 6):
                    ...
                    if v.name == 'net/W:0':
                        var_rdm.append(collection_vars_net[i])
                        self._log.print3(str(collection_vars_net[i].shape))
                        collection_vars_net.pop(i)
                        i = i - 1
                    if v.name == 'net/b:0':
                        var_rdm.append(collection_vars_net[i])
                        self._log.print3(str(collection_vars_net[i].shape))
                ...
                var_rdm.append(collection_vars_net[-1])
                tf.variables_initializer(var_rdm).run()
                collection_vars_net.pop(len(collection_vars_net) - 1) ## randomly initialize b_3, the bias in softmax layer
                
                tf.variables_initializer([collection_vars_net[-1]]).run()
                collection_vars_net.pop(len(collection_vars_net) - 1)## randomly initialize W_26, [2, 250, 1, 1, 1], weight of the last layer
                
                reset_trainer = True
                #self._log.print3(str(collection_vars_net))
