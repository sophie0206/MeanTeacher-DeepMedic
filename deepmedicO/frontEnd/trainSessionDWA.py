...
# For Depth-Width-augmented Method!!!!!!!!!!!11
...
from deepmedicO.neuralnet.cnn3dDWA import Cnn3d
    ...
    def run_session(self, *args):
        ...
        with tf.Session( graph=graphTf, config=tf.ConfigProto(log_device_placement=False, device_count={'CPU':999, 'GPU':99}, gpu_options=tf.GPUOptions(allow_growth=True)) ) as sessionTf:
            ...
            if file_to_load_params_from is not None: # Load params
                ...
                rdmInit_vars = collection_vars_net[-20:] ## get the last two, softmax layer parameters.and also random initialize the plus units weights.
                tf.variables_initializer(rdmInit_vars).run()
                
                l_i = 0
                r_i = 1
                self._log.print3(str(r_i) + " Channel training\n Depth-Width Augmented Method!")
                for key in var:
                    if key == 'net/W':
                        W0 = reader.get_tensor(key)[:, l_i:r_i, :, :, :]
                    if key == 'net/b':
                        b0 = reader.get_tensor(key)[l_i:r_i]
                    if key == 'net/W_8':
                        W8 = reader.get_tensor(key)[:, l_i:r_i, :, :, :]
                    if key == 'net/b_1':
                        b1 = reader.get_tensor(key)[l_i:r_i]
                    if key == 'net/W_16':
                        W16 = reader.get_tensor(key)[:, l_i:r_i, :, :, :]
                    if key == 'net/b_2':
                        b2 = reader.get_tensor(key)[l_i:r_i]
                    if key == 'net/W_25':  #########################Not Sure!!!!!!!!!!! And delete the corresponding W25 in stroke model variables.
                        W25 = reader.get_tensor(key)
                        self._log.print3("net/W_25")
                        sessionTf.run(tf.assign(collection_vars_net[-20][:250, :, :, :, :], W25))
                    if key == 'net/gBn_23':
                        gBn23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-18][:250], gBn23))
                        self._log.print3("net/gBn_23")
                    if key == 'net/bBn_23':
                        bBn23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-17][:250], bBn23))
                        self._log.print3("net/bBn_23")
                    if key == 'net/muBnsForRollingAverage_23':
                        mu23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-16][:, :250], mu23))
                        self._log.print3("net/muBnsForRollingAverage_23")
                    if key == 'net/varBnsForRollingAverage_23':
                        var23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-15][:, :250], var23))
                        self._log.print3("net/varBnsForRollingAverage_23")
                    if key == 'net/sharedNewMu_B_23':
                        sMu23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-14][:250], sMu23))
                        self._log.print3("net/sharedNewMu_B_23")
                    if key == 'net/sharedNewVar_B_23':
                        sVar23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-13][:250], sVar23))
                        self._log.print3("net/sharedNewVar_B_23")
                    if key == 'net/aPrelu_23':
                        aP23 = reader.get_tensor(key)
                        sessionTf.run(tf.assign(collection_vars_net[-12][:250], aP23))
                        self._log.print3("net/aPrelu_23")
                ...
                saver_net = tf.train.Saver( var_list=collection_vars_net[:-20] ) # Used to load the net's parameters.
