...
# Classic Fine-tune, no augmentation. use average of four channels
...
class TrainSession(Session):
    ...
    def run_session(self, *args):
        ...
        with tf.Session( graph=graphTf, config=tf.ConfigProto(log_device_placement=False, device_count={'CPU':999, 'GPU':99}, gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.5)) ) as sessionTf:
