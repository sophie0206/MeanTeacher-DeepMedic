...
from deepmedicO.neuralnet.trainerBSS import Trainer
            ...
            if file_to_load_params_from is not None: # Load params
                ...
                for key in var:
                    ...
                    if key == 'net/W_25':  #########################Not Sure!!!!!!!!!!! And delete the corresponding W25 in stroke model variables.
                        ...
                        assign_ops.append(tf.assign(collection_vars_net[-11][:fms, :, :, :, :], W25))
                    if key == 'net/gBn_23':
                        gBn23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-9][:fms], gBn23))
                    if key == 'net/bBn_23':
                        bBn23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-8][:fms], bBn23))
                    if key == 'net/muBnsForRollingAverage_23':
                        mu23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-7][:, :fms], mu23))
                    if key == 'net/varBnsForRollingAverage_23':
                        var23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-6][:, :fms], var23))
                    if key == 'net/sharedNewMu_B_23':
                        sMu23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-5][:fms], sMu23))
                    if key == 'net/sharedNewVar_B_23':
                        sVar23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-4][:fms], sVar23))
                    if key == 'net/aPrelu_23':
                        aP23 = reader.get_tensor(key)
                        assign_ops.append(tf.assign(collection_vars_net[-3][:fms], aP23))
