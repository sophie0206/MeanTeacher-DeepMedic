...
from deepmedicO.neuralnet.pathwayTypes import PathwayTypes
from deepmedicO.neuralnet.utils import calcRecFieldFromKernDimListPerLayerWhenStrides1
from deepmedicO.neuralnet.layers import ConvLayer, LowRankConvLayer
...
##==================================================ADDED==========================================##
def get_L2_normalized(inputToNextLayerTrain, inputToNextLayerVal, inputToNextLayerTest, layer):
    ##In what area to calculate the L2 value, does that matter????
    ## 2019.03.19.14.04: changed to feature map region/scale.
    shapeTrain = layer.outputShape["train"]
    shapeVal = layer.outputShape["val"]
    shapeTest = layer.outputShape["test"]

    h_reTrain = tf.reshape(inputToNextLayerTrain, shape=[shapeTrain[0]*shapeTrain[1], shapeTrain[2], shapeTrain[3], shapeTrain[4]])
    h_reVal = tf.reshape(inputToNextLayerVal, shape=[shapeVal[0]*shapeVal[1], shapeVal[2], shapeVal[3], shapeVal[4]])
    h_reTest = tf.reshape(inputToNextLayerTest, shape=[shapeTest[0]*shapeTest[1], shapeTest[2], shapeTest[3], shapeTest[4]])

    #h_plus_re = tf.reshape(h, shape=[shape[0]*shape[1], shape[2], shape[3], shape[4]])
    normHTrain = tf.reshape(tf.nn.l2_normalize(h_reTrain, axis=0), shape=[shapeTrain[0], shapeTrain[1], shapeTrain[2], shapeTrain[3], shapeTrain[4]])
    normHVal = tf.reshape(tf.nn.l2_normalize(h_reVal, axis=0), shape=[shapeVal[0], shapeVal[1], shapeVal[2], shapeVal[3], shapeVal[4]])
    normHTest = tf.reshape(tf.nn.l2_normalize(h_reTest, axis=0), shape=[shapeTest[0], shapeTest[1], shapeTest[2], shapeTest[3], shapeTest[4]])
    
    initV = np.ones((shapeTrain[0], shapeTrain[1], 1, 1, 1), dtype="float32") * 20
    #initV = np.asarray( rng.normal(loc=15, scale=30, size=(layer.outputShape["train"][0], layer.outputShape["train"][1], 1, 1, 1)), dtype='float32' )
    gamma = tf.Variable(initV, dtype="float32", name='gamma')
    layer.params = layer.params + [gamma]
    print(str(layer.params))
    #gamma2 = tf.Variable(initV, dtype="float32", name='gammaPlus')
    scaledHTrain = gamma * normHTrain
    scaledHVal = gamma * normHVal
    scaledHTest = gamma * normHTest
    
    print("\n\nIn the Fc layer to be widened\n"+ str(gamma))
    
    return (scaledHTrain, scaledHVal, scaledHTest)
...
class Pathway(object):
    ...
    def makeLayersOfThisPathwayAndReturnDimensionsOfOutputFM(self,
        ...
        for layer_i in range(0, numOfLayers) :
            ...
            ######============================================================ADDED==================================##
            if (layer_i == 1 or layer_i == 2) and (self.getStringType() == "FC"):
                (inputToNextLayerTrain, 
                 inputToNextLayerVal, 
                 inputToNextLayerTest) = get_L2_normalized(inputToNextLayerTrain, 
                                                           inputToNextLayerVal, 
