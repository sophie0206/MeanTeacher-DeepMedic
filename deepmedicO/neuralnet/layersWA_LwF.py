...
from deepmedicO.neuralnet.ops import applyDropout, makeBiasParamsAndApplyToFms, applyRelu, applyPrelu, applyElu, applySelu, pool3dMirrorPad
...
##For Width Agumented and Learning without Forgetting!!!
...
##===============================================================================================================##
    def _createWeightsTensorAndConvolveLwF(self, rng, filterShape, convWInitMethod, 
                                        inputToConvTrain, inputToConvVal, inputToConvTest,
                                        inputToConvShapeTrain, inputToConvShapeVal, inputToConvShapeTest) :
        #-----------------------------------------------
        #------------------ Convolution ----------------
        ...
        #----- Initialise the weights -----
        # W shape: [#FMs of this layer, #FMs of Input, rKernDim, cKernDim, zKernDim]
        filterShape1 = [5, filterShape[1], filterShape[2], filterShape[3], filterShape[4]]
        filterShape2 = [2, filterShape[1], filterShape[2], filterShape[3], filterShape[4]]

        Wold = createAndInitializeWeightsTensor(filterShape1, convWInitMethod, rng)
        self._Wnew = createAndInitializeWeightsTensor(filterShape2, convWInitMethod, rng)
        self._W = tf.concat([Wold, self._Wnew], axis=0)
        self.params = [self._W] + self.params
    ...
    def makeLayer(self,
        ...
        ##========================================================================================================##
        if isLastLayer:
            tupleWithOuputAndShapeTrValTest = self._createWeightsTensorAndConvolveLwF( rng, filterShape, convWInitMethod, 
                                                                                inputToConvTrain, inputToConvVal, inputToConvTest,
                                                                                inputToConvShapeTrain, inputToConvShapeVal, inputToConvShapeTest)
        else:
...
class SoftmaxLayer(TargetLayer):
    ...
    def makeLayer(  self,
        ...
        #==========================================================================##
        self._Wnew = layerConnected._Wnew
        print(self._Wnew)
        ...
        (self._b1,
        biasedInputToSoftmaxTrain1,
        biasedInputToSoftmaxVal1,
        biasedInputToSoftmaxTest1) = makeBiasParamsAndApplyToFms( self.input["train"][:, :5, :, :, :], self.input["val"][:, :5, :, :, :], self.input["test"][:, :5, :, :, :], 5 )
        
        (self._b2,
        biasedInputToSoftmaxTrain2,
        biasedInputToSoftmaxVal2,
        biasedInputToSoftmaxTest2) = makeBiasParamsAndApplyToFms( self.input["train"][:, 5:, :, :, :], self.input["val"][:, 5:, :, :, :], self.input["test"][:, 5:, :, :, :], 2 )
        
        self.params = self.params + [self._b1] + [self._b2]

        biasedInputToSoftmaxTrain = tf.concat([biasedInputToSoftmaxTrain1, biasedInputToSoftmaxTrain2], axis=1)
        biasedInputToSoftmaxVal = tf.concat([biasedInputToSoftmaxVal1, biasedInputToSoftmaxVal2], axis=1)
        biasedInputToSoftmaxTest = tf.concat([biasedInputToSoftmaxTest1, biasedInputToSoftmaxTest2], axis=1)
        ##=======================================================================================================##
