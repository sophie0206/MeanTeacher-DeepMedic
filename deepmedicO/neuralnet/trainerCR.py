...
class Trainer(object):
    ...
    def _setup_costs(self, log):
        ...
        if "xentr" in self._losses_and_weights and self._losses_and_weights["xentr"] is not None:
            ...
            Oshape = self._net.finalTargetLayer.outputShape["train"]
            reshapedOutput = tf.reshape(self._net.finalTargetLayer.output["train"], shape=[Oshape[0], Oshape[2], Oshape[3], Oshape[4]])
            #cost += tf.reduce_mean(tf.squared_difference(tf.reshape(self._net.finalTargetLayer.output["train"], shape=[Oshape[0], Oshape[2], Oshape[3], Oshape[4]]) , y_gt))
            y_gt = tf.cast(y_gt, tf.float32)
            #cost += tf.nn.l2_loss(reshapedOutput - y_gt) #???????????????????????????
            cost += tf.reduce_mean((reshapedOutput - y_gt) ** 2)
