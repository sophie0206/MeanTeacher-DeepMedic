...
import deepmedicO.neuralnet.optimizers as optimizers_dm
import deepmedicO.neuralnet.cost_functions as cfs
...
class Trainer(object):
    ...
    def __init__(   self,
        ...
        # Params for costs
        ##==========================================ADDED================================================##
        self._y_old = tf.argmax(self._net.finalTargetLayer.p_y_given_x_train[:, :5, :, :, :], axis=1)
        ##===============================================================================================##
    ...
    def _setup_costs(self, log):
        ...
        if "xentr" in self._losses_and_weights and self._losses_and_weights["xentr"] is not None:
            ...
            w_per_cl_vec = self._compute_w_per_class_vector_for_xentr( self._net.num_classes - 5, y_gt )
            cost += self._losses_and_weights["xentr"] * cfs.x_entr( self._net.finalTargetLayer.p_y_given_x_train[:, 5:, :, :, :], y_gt, w_per_cl_vec )
        ...
        ##?????????????????????????????????????????????????????????????????????????
        #if tf.subtract(self._num_epochs_trained_tfv, tf.Variable(0, dtype="int64") == 0:
         #   self.y_old = self._net.finalTargetLayer.p_y_given_x_train[:, :5, :, :, :]
            
        old_cost = cfs.dsc( self._net.finalTargetLayer.p_y_given_x_train[:, :5, :, :, :], self._y_old )
        lambdaO = tf.constant(1.0)
        
        self._total_cost = cost + lambdaO * old_cost
    ...
    def create_optimizer(   self,
        ...
        new_params = [self._net.finalTargetLayer._Wnew] + [self._net.finalTargetLayer._b2]
        all_params = self._net.get_trainable_params(log, self._indicesOfLayersPerPathwayTypeToFreeze)
        params_to_opt = tf.cond(self._num_epochs_trained_tfv < warmUpLen, lambda: new_params, lambda: all_params)
        ##================================================================================================##
