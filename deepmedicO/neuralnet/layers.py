...
from deepmedicO.neuralnet.ops import applyDropout, makeBiasParamsAndApplyToFms, applyRelu, makeDDBiasParamsAndApplyToFms, applyPrelu, applyElu, applySelu, pool3dMirrorPad
from deepmedicO.neuralnet.ops import applyBn, createAndInitializeWeightsTensor, convolveWithGivenWeightMatrix, applySoftmaxToFmAndReturnProbYandPredY
...
class SoftmaxLayer(TargetLayer):
    ...
    def makeReconstructionLayer(  self,
                    rng,
                    layerConnected, # the basic layer, at the output of which to connect this softmax.
                    softmaxTemperature = 1):
        ...
        # ============ Softmax ==============
        #self.p_y_given_x_2d_train = ? Can I implement negativeLogLikelihood without this ?
        ( self._aPrelu, ouputTrain, outputVal, outputTest ) = applyPrelu(biasedInputToSoftmaxTrain, biasedInputToSoftmaxVal, biasedInputToSoftmaxTest, self._numberOfOutputClasses)
        self.params = self.params + [self._aPrelu]

        self._setBlocksOutputAttributes(ouputTrain, outputVal, outputTest, self.inputShape["train"], self.inputShape["val"], self.inputShape["test"])
    ...
    def getRpRnTpTnForTrain0OrVal1_mixup(self, y, training0OrValidation1, batchSize, lam):
        # The returned list has (numberOfClasses)x4 integers: >numberOfRealPositives, numberOfRealNegatives, numberOfTruePredictedPositives, numberOfTruePredictedNegatives< for each class (incl background).
        # Order in the list is the natural order of the classes (ie class-0 RP,RN,TPP,TPN, class-1 RP,RN,TPP,TPN, class-2 RP,RN,TPP,TPN ...)
        # param y: y = T.itensor4('y'). Dimensions [batchSize, r, c, z]
        #y = tf.cast(y, tf.float32)
        #y = lam * y[:batchSize] + (1-lam) * y[batchSize:]
        #y = y[:batchSize]
        yPredToUse = self.y_pred_train if  training0OrValidation1 == 0 else self.y_pred_val
