...
from deepmedicO.neuralnet.layersWA_LwF import ConvLayer, LowRankConvLayer
...
##==================================================ADDED==========================================##
def L2_normalize(h, h_plus, shape):
    ##In what area to calculate the L2 value, does that matter????
    for i in range(shape[0]):
        normVal = np.linalg.norm(h[i, :, :, :, :], keepdims=True)
        normValp = np.linalg.norm(h_plus[i, :, :, :, :], keepdims=True)
        print("L2 Norm Value: " + str(normVal)+"\n")
        if i == 0:
            concatenatedH = normVal
            concatenatedHp = normValp
        else:
            concatenatedH = tf.concat([concatenatedH, nomrVal])
            concatenatedHp = tf.concat([concatenatedHp, nomrValp])

    concatenatedH = tf.reshape(concatenatedH, shape=[shape[0], 1, 1, 1, 1])
    concatenatedHp = tf.reshape(concatenatedHp, shape=[shape[0], 1, 1, 1, 1])

    normH = h / concatenatedH
    normHp = h_plus / concatenatedHp
        ...
        for layer_i in range(0, numOfLayers) :
            ...
            if layer_i not in indicesOfLayersToConnectResidualsInOutputForPathway : #not a residual connecting here
                ...
                if (layer_i == 1) and (self.getStringType() == "FC"):
                    normH, normHp = L2_normalize(layer.output["train"][:, :250, :, :, :], layer.output["train"][:, 250:, :, :, :], layer.outputShape["train"])
                    ##???????????????????????????????
                    initV = np.asarray( rng.normal(loc=15, scale=30, size=(layer.outputShape["train"][0], layer.outputShape["train"][1], 1, 1, 1)), dtype='float32' )
                    gamma1 = tf.Variable(initV, dtype="float32", name='gammaOriginal')
                    gamma2 = tf.Variable(initV, dtype="float32", name='gammaPlus')
                    scaledH = gamma1 * normH
                    scaledHp = gamma2 * normHp
                    layer.params = layer.params + [gamma1] + [gamma2]
                    log.print3("In the Fc layer to be widened\n")
                    
                    inputToNextLayerTrain = tf.concat([scaledH, scaledHp], axis=1)
                ###=======================================================================================================##
