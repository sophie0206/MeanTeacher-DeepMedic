...
class Trainer(object):
    ...
    def _setup_costs(self, log):
        ...
        if "xentr" in self._losses_and_weights and self._losses_and_weights["xentr"] is not None:
            ...
            w_per_cl_vec = self._compute_w_per_class_vector_for_xentr( self._net.num_classes, y_gt[:self._net.batchSize["train"]//2] )
            ##========================================================================================================##
            ### does this w_per_class_vec matter? for class imbalance problem
            #lam = self._net._lam
            #mixup_loss = lam * cfs.x_entr_mixup( self._net.finalTargetLayer.p_y_given_x_train, y_gt[:self._net.batchSize["train"]], w_per_cl_vec ) +(1-lam) * cfs.x_entr_mixup(self._net.finalTargetLayer.p_y_given_x_train, y_gt[self._net.batchSize["train"]:], w_per_cl_vec ) 
            
            log.print3(str(tf.shape(y_gt)))
            normal_loss = cfs.x_entr(self._net.finalTargetLayer.p_y_given_x_train[:(self._net.batchSize["train"]//2)], y_gt[:(self._net.batchSize["train"]//2)], w_per_cl_vec)
            mixup_loss = cfs.x_entr_mixup(self._net.finalTargetLayer.p_y_given_x_train[(self._net.batchSize["train"]//2):], y_gt[(self._net.batchSize["train"]//2):])
            
            cost += self._losses_and_weights["xentr"] * (mixup_loss + normal_loss)
            ##=========================================================================================================##
